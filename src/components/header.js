...
const Menu = styled.div`
  ${tw`w-full bg-black flex flex-col items-end lg:flex lg:items-center lg:justify-around lg:flex-row lg:w-1/5 lg:visible`}
...
const Header = () => {
  const [visible, setVisible] = useState(is.mobile() ? false : true);

  return (
    <Nav>
      <img src={url + (is.safari() ? '.png' : '.webp')} alt="Wally Pankratz signature" className="h-full ml-3" />
      <div className="block mr-3 md:hidden">
        <button id="hamburger" className="flex items-center px-3 py-2 border rounded border-white text-white" onClick={() => {setVisible(!visible); console.log(visible, is.mobile());}}>
